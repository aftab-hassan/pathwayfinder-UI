...
for(var i=0;i<Object.keys(buffer[0].patient[index].scenario[0].nodes).length;i++)
{
 ...
 //alert("weight==["+buffer[0].patient[index].scenario[0].nodes[i].probability+"]");

 weightlabel = buffer[0].patient[index].scenario[0].nodes[i].probability; 
 //alert(weightlabel.toString());
 //alert(weightlabel+" and "+weightlabel.charAt(1)); 
 //alert("typeof(weightlabel)=="+typeof(weightlabel));
 
 if(weightlabel)
 {
  //alert("came inside not undefined,weight==["+buffer[0].patient[index].scenario[0].nodes[i].probability+"]");
  weightsarray.push(weightlabel);
 }
 else
 {
  //alert("came inside undefined,weight==["+weightlabel+"]");
  weightsarray.push("");
 }
}
...
invisiblestyle = invisiblestyle + 0 + 'px';
...
renderer.drawNodes(function(graph, root) 
{
   ...
   if(usercount == 1)
   {
    ...
    userfrom = d;
    $().toastmessage('showNoticeToast', 'Please select one of the grayed out nodes to draw an edge between the two!');
   ...
   else if(usercount == 2)
   {
    ...
    {
     alert("involvednodesstring == "+involvednodesstring);
     ...
     alert('now what should happen is, it should look at the key-value store, find which is the scenario which includes all of these nodes plus, DXafta, and then get the json, and redraw the diff. For now, there\'s no lookup, I\'m just hardcoding to draw scenario0 on clicking this OK button. ')
     MyFunc(0);
    ...
    {
     usercount = 1;
     //$().toastmessage('showSuccessToast', displaytext);
     //$().toastmessage('showNoticeToast', displaytext);
     //$().toastmessage('showWarningToast', displaytext);
     $().toastmessage('showErrorToast', 'this node cannot be chosen!');
